...
Additionally, notice the return type of the method above: Spring REST API methods will (typically) return a `ResponseEntity`, which is a Spring data structure that is automatically serialized to JSON when sent back to the client.  Why a `ResponsEntity` isntead of just the type we want to return?  This allows us to handle error conditions more gracefully, because a ResponseEntity can indicate an error instead of just returning `null`.  The one exception to this rule is that a "get all" (such as the `GET /recipes/` mentioned above) will return a `List` of the appropriate type, which will again automatically be serialized to JSON.    An empty list is an entirely valid response from a "get all" query so this can be used if anything goes wrong.
